...
p2c <- ggplot() +
  ...
  annotate("text", x = 2.2, y = -1.1, label = paste0("Candidate Targets\n(n=", length(ezhu_set), ")"), size = 3.6, color = "#00A087FF") +
  ...
  coord_equal() +
  theme_void() +
  labs(title = "DEG ∩ Ferroptosis ∩ Candidate Targets")
...
if (file.exists(boltz_file)) {
  ...
  if (nrow(boltz) > 0) {
    ...
    score_title <- if (score_higher_better) "Boltz-2 Predicted pIC50 (higher is better)" else "Boltz-2 Affinity (lower is better)"
    score_xlabel <- if (score_higher_better) "Predicted pIC50 (higher is better)" else "Affinity score (lower is better)"

    boltz_matrix <- reshape2::dcast(boltz, Protein ~ Ligand, value.var = score_col)
    rownames(boltz_matrix) <- boltz_matrix$Protein
    boltz_matrix$Protein <- NULL
    boltz_long <- reshape2::melt(as.matrix(boltz_matrix))
    colnames(boltz_long) <- c("Protein", "Ligand", "Score")

    protein_order <- aggregate(Score ~ Protein, boltz_long, mean, na.rm = TRUE)
    protein_order <- if (score_higher_better) {
      protein_order[order(-protein_order$Score), "Protein"]
    } else {
      protein_order[order(protein_order$Score), "Protein"]
    }
    ligand_order <- aggregate(Score ~ Ligand, boltz_long, mean, na.rm = TRUE)
    ...
    score_min <- min(boltz_long$Score, na.rm = TRUE)
    score_max <- max(boltz_long$Score, na.rm = TRUE)
    midpoint <- (score_min + score_max) / 2

    boltz_long$Label <- ifelse(is.na(boltz_long$Score), "NA", sprintf("%.2f", boltz_long$Score))
    p6c <- ggplot(boltz_long, aes(x = Ligand, y = Protein, fill = Score)) +
      geom_tile(color = "white", linewidth = 0.4) +
      geom_text(aes(label = Label), size = 2.4, color = "white", fontface = "bold") +
      scale_fill_gradient2(low = "#4575B4", mid = "#FEE090", high = "#D73027", midpoint = midpoint, na.value = "#D0D5DA") +
      scale_x_discrete(labels = ligand_labels) +
      theme_pub() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7)) +
      labs(title = score_title, x = "Ligand", y = "Protein")
    ...
    modelgene_file <- file.path(res_dir, "boltz2_affinity_results_modelgenes.csv")
    if (file.exists(modelgene_file)) {
      modelgene <- read.csv(modelgene_file) %>% filter(Protein %in% c("SLC27A5", "ENO1"))
      modelgene_score_col <- if ("Affinity_pIC50" %in% names(modelgene)) "Affinity_pIC50" else "Affinity_Pred_Value"
      modelgene_ok <- modelgene %>% filter(Status == "OK")
      if (nrow(modelgene_ok) > 0) {
        modelgene_ligand_order <- modelgene_ok %>%
          group_by(Ligand) %>%
          dplyr::summarise(mean_score = mean(.data[[modelgene_score_col]], na.rm = TRUE), .groups = "drop") %>%
          arrange(desc(mean_score)) %>%
          pull(Ligand)
        modelgene_ligand_order <- as.character(modelgene_ligand_order)
        modelgene_labels <- setNames(
          ifelse(nchar(modelgene_ligand_order) > 18, paste0(substr(modelgene_ligand_order, 1, 15), "..."), modelgene_ligand_order),
          modelgene_ligand_order
        )
        modelgene$Protein <- factor(modelgene$Protein, levels = c("SLC27A5", "ENO1"))
        modelgene$Ligand <- factor(modelgene$Ligand, levels = modelgene_ligand_order)
        modelgene$Label <- ifelse(is.na(modelgene[[modelgene_score_col]]), "NA", sprintf("%.2f", modelgene[[modelgene_score_col]]))
        p6e <- ggplot(modelgene, aes(x = Ligand, y = Protein, fill = .data[[modelgene_score_col]])) +
          geom_tile(color = "white", linewidth = 0.4) +
          geom_text(aes(label = Label), size = 2.4, color = "white", fontface = "bold") +
          scale_fill_gradient2(low = "#4575B4", mid = "#FEE090", high = "#D73027",
                               midpoint = mean(modelgene_ok[[modelgene_score_col]], na.rm = TRUE),
                               na.value = "#D0D5DA") +
          scale_x_discrete(labels = modelgene_labels) +
          theme_pub() +
          theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7)) +
          labs(title = "Model-gene Boltz-2 pIC50", x = "Ligand", y = "Model gene")
      } else {
        p6e <- ggplot() + annotate("text", x=0.5, y=0.5, label="Model-gene Boltz-2\n(Not Available)", size=5) + theme_void()
      }
    } else {
    ...
    if (file.exists(dock_file)) {
      ...
      if (nrow(merged) > 0) {
        ...
        cor_val <- cor(merged$Vina_Pos, merged[[score_col]], method = "spearman")
        p_val <- cor.test(merged$Vina_Pos, merged[[score_col]], method = "spearman")$p.value
        p6f <- ggplot(merged, aes(x = Vina_Pos, y = .data[[score_col]], color = Protein)) +
          geom_point(alpha = 0.7) +
          geom_smooth(method = "lm", color = "black", linewidth = 0.6) +
          annotate("text", x = min(merged$Vina_Pos), y = max(merged[[score_col]]),
                   hjust = 0, vjust = 1, size = 3.2,
                   label = sprintf("Spearman r=%.3f, P=%.2e", cor_val, p_val)) +
          scale_color_cool() +
          theme_pub() +
          labs(title = "Vina vs Boltz-2 Consistency", x = "-Vina Score (higher is better)", y = score_xlabel)
      } else {
